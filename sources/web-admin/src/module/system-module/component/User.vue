      ...
      <el-col :span="12">
        <el-form-item label="所属角色" prop="roles">
          <el-select multiple
                     ...
                     placeholder="请选择"
                     v-model="user.roles">
            <el-option-group v-for="{id,name,children:roles} in roleTree"
                             :key="id"
                             :label="name">
...
<script>
  ...
  export default class User extends Vue {
    ...
    submit () {
      let validations = [this.$refs.userform.validate()]
      if (this.$refs.postForms) {
        validations = validations.concat(this.$refs.postForms.map(form => form.validate()))
      }
