...
<script>
  ...
  import config from 'element-datatables/src/components/config'
    ...
    methods: {
      ...
      reloadAjaxData () {
        ...
        if (typeof (this.ajax) === 'string') {
          ajax.url = this.ajax
          ajax.params = this.setReturnData ? {}: Object.assign({ page: this.currentPage - 1, size: this.pageSize }, draw, this.serverParams)
        } else {
          if (!this.ajax.url) {
        ...
        if (!ajax.url || ajax.url.search('undefined') >= 0) {
          ...
          config.$http(transelateAjax(ajax)).then(response => {
            ...
            if (this.setReturnData) {
              let flag = 0
              let max = this.pageSize * this.currentPage
              this.tableData =  this.setReturnData(response).filter(ele =>{
                let temp = flag< max && flag >= max - this.pageSize
                return temp
              })
              this.success = true
              return
