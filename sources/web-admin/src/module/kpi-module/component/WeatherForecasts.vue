<template>
  <div>
    ...
    <el-row>
      <el-col :span="24">
        <ele-data-tables ref="list"
          ...
          <el-table-column prop="predictDate" label="日期" sortable="custom">
            <template v-slot="{row:{predictDate}}">{{ toLocalDate(predictDate) }}</template>
          </el-table-column>
    ...
    <el-dialog v-if="visible" :visible.sync="visible" title="指标编辑">
      <weather-forecast v-model="selectedRow" />
...
<script>
  ...
  export default class WeatherForecasts extends Mixins(BaseList) {
    get baseModule () { return 'kpi/weatherForecast' }
  }
</script>
