<template>
  <div>
    ...
    <el-row>
      <el-col :span="24">
        <ele-data-tables ref="list"
          ...
          <el-table-column prop="name" label="年龄段类型" sortable="custom" />
    ...
    <el-dialog v-if="visible" :visible.sync="visible" title="指标编辑">
      <month-tourist-age v-model="selectedRow" />
...
<script>
  ...
  import MonthTouristAge from './MonthTouristAge'

  @Component({
    components: { MonthTouristAge }
  })
  export default class MonthTouristAges extends Mixins(BaseList) {
    get baseModule () { return 'kpi/monthTouristAge' }
  }
</script>
