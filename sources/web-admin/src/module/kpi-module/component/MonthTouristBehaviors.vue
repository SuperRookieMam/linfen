<template>
  <div>
    ...
    <el-row>
      <el-col :span="24">
        <ele-data-tables ref="list"
          ...
          <el-table-column prop="name" label="行为" sortable="custom" />
    ...
    <el-dialog v-if="visible" :visible.sync="visible" title="指标编辑">
      <month-tourist-behavior v-model="selectedRow" />
...
<script>
  ...
  import MonthTouristBehavior from './MonthTouristBehavior'

  @Component({
    components: { MonthTouristBehavior }
  })
  export default class MonthTouristBehaviors extends Mixins(BaseList) {
    get baseModule () { return 'kpi/monthTouristBehavior' }
  }
</script>
