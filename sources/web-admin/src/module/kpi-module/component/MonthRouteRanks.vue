<template>
  <div>
    ...
    <el-row>
      <el-col :span="24">
        <ele-data-tables ref="list"
          ...
          <el-table-column prop="name" label="线路名称" sortable="custom" />
    ...
    <el-dialog v-if="visible" :visible.sync="visible" title="指标编辑">
      <month-route-rank v-model="selectedRow" />
...
<script>
  ...
  import MonthRouteRank from './MonthRouteRank'

  @Component({
    components: { MonthRouteRank }
  })
  export default class MonthRouteRanks extends Mixins(BaseList) {
    get baseModule () { return 'kpi/monthRouteRank' }
  }
</script>
