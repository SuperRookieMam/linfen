<template>
  <div>
    ...
    <el-row>
      <el-col :span="24">
        <ele-data-tables ref="list"
          ...
          <el-table-column prop="name" label="资源名称" sortable="custom" />
    ...
    <el-dialog v-if="visible" :visible.sync="visible" title="指标编辑">
      <year-resource v-model="selectedRow" />
...
<script>
  ...
  import YearResource from './YearResource'

  @Component({
    components: { YearResource }
  })
  export default class YearResources extends Mixins(BaseList) {
    get baseModule () { return 'kpi/yearResource' }
  }
</script>
