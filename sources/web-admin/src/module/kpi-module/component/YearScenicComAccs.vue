<template>
  <div>
    ...
    <el-row>
      <el-col :span="24">
        <ele-data-tables ref="list"
          ...
          <el-table-column prop="complaintNumber" label="投诉量" sortable="custom" />
          <el-table-column prop="acceptanceNumber" label="受理量" sortable="custom" />
    ...
    <el-dialog v-if="visible" :visible.sync="visible" title="指标编辑">
      <year-scenic-com-acc v-model="selectedRow" />
...
<script>
  ...
  import YearScenicComAcc from './YearScenicComAcc'

  @Component({
    components: { YearScenicComAcc }
  })
  export default class YearScenicComAccs extends Mixins(BaseList) {
    get baseModule () { return 'kpi/yearScenicComAcc' }
  }
</script>
