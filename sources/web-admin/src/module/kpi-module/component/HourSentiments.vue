<template>
  <div>
    ...
    <el-row>
      <el-col :span="24">
        <ele-data-tables ref="list"
          ...
          <el-table-column prop="name" label="舆情标题" sortable="custom" />
    ...
    <el-dialog v-if="visible" :visible.sync="visible" title="指标编辑">
      <hour-sentiment v-model="selectedRow" />
...
<script>
  ...
  import HourSentiment from './HourSentiment'

  @Component({
    components: { HourSentiment }
  })
  export default class HourSentiments extends Mixins(BaseList) {
    get baseModule () { return 'kpi/hourSentiment' }
  }
</script>
