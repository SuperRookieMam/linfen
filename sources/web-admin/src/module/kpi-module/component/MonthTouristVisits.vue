<template>
  <div>
    ...
    <el-row>
      <el-col :span="24">
        <ele-data-tables ref="list"
          ...
          <el-table-column prop="name" label="频次" sortable="custom" />
    ...
    <el-dialog v-if="visible" :visible.sync="visible" title="指标编辑">
      <month-tourist-visit v-model="selectedRow" />
...
<script>
  ...
  import MonthTouristVisit from './MonthTouristVisit'

  @Component({
    components: { MonthTouristVisit }
  })
  export default class MonthTouristVisits extends Mixins(BaseList) {
    get baseModule () { return 'kpi/monthTouristVisit' }
  }
</script>
