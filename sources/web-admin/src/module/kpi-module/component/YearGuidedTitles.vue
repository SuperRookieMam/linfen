<template>
  <div>
    ...
    <el-row>
      <el-col :span="24">
        <ele-data-tables ref="list"
          ...
          <el-table-column prop="name" label="职称" sortable="custom" />
    ...
    <el-dialog v-if="visible" :visible.sync="visible" title="指标编辑">
      <year-guided-title v-model="selectedRow" />
...
<script>
  ...
  import YearGuidedTitle from './YearGuidedTitle'

  @Component({
    components: { YearGuidedTitle }
  })
  export default class YearGuidedTitles extends Mixins(BaseList) {
    get baseModule () { return 'kpi/yearGuidedTitle' }
  }
</script>
