<template>
  <div>
    ...
    <el-row>
      <el-col :span="24">
        <ele-data-tables ref="list"
          ...
          <el-table-column prop="name" label="性别" sortable="custom" />
    ...
    <el-dialog v-if="visible" :visible.sync="visible" title="指标编辑">
      <month-tourist-gender v-model="selectedRow" />
...
<script>
  ...
  import MonthTouristGender from './MonthTouristGender'

  @Component({
    components: { MonthTouristGender }
  })
  export default class MonthTouristGenders extends Mixins(BaseList) {
    get baseModule () { return 'kpi/monthTouristGender' }
  }
</script>
