<template>
  <div>
    ...
    <el-row>
      <el-col :span="24">
        <ele-data-tables ref="list"
          ...
          <el-table-column prop="name" label="星级" sortable="custom" />
    ...
    <el-dialog v-if="visible" :visible.sync="visible" title="指标编辑">
      <month-tourist-check-in v-model="selectedRow" />
...
<script>
  ...
  import MonthTouristCheckIn from './MonthTouristCheckIn'

  @Component({
    components: { MonthTouristCheckIn }
  })
  export default class MonthTouristCheckIns extends Mixins(BaseList) {
    get baseModule () { return 'kpi/monthTouristCheckIn' }
  }
</script>
