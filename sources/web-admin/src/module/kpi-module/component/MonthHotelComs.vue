<template>
  <div>
    ...
    <el-row>
      <el-col :span="24">
        <ele-data-tables ref="list"
          ...
          <el-table-column prop="name" label="酒店名称" sortable="custom" />
          <el-table-column prop="value" label="值" sortable="投诉量" />
    ...
    <el-dialog v-if="visible" :visible.sync="visible" title="指标编辑">
      <month-hotel-com v-model="selectedRow" />
...
<script>
  ...
  import MonthHotelCom from './MonthHotelCom'

  @Component({
    components: { MonthHotelCom }
  })
  export default class MonthHotelComs extends Mixins(BaseList) {
    get baseModule () { return 'kpi/monthHotelCom' }
  }
</script>
