<template>
  <div>
    ...
    <el-row>
      <el-col :span="24">
        <ele-data-tables ref="list"
          ...
          <el-table-column prop="name" label="城市名称" sortable="custom" />
    ...
    <el-dialog v-if="visible" :visible.sync="visible" title="指标编辑">
      <month-city v-model="selectedRow" />
...
<script>
  ...
  import MonthCity from './MonthCity'

  @Component({
    components: { MonthCity }
  })
  export default class MonthCitys extends Mixins(BaseList) {
    get baseModule () { return 'kpi/monthCity' }
  }
</script>
